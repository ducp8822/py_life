...
from algo.sequence_analysis import (
    get_basic_stats,
    find_orfs,
    translate_sequence,
    validate_sequence,
            ...
            <div class='hero-grid'>
                ...
                <div class='hero-stat-grid'>
                    <div class='hero-stat'>
                        ...
                        <div class='hero-stat-sub'>Kiểm tra cấu hình NCBI trước khi chạy các truy vấn trực tiếp.</div>
                    </div>
                    <div class='hero-stat'>
                        <div class='hero-stat-label'>Giới hạn tìm kiếm</div>
                        <div class='hero-stat-value'>{max_results}</div>
                        <div class='hero-stat-sub'>Số bản ghi tối đa cho mỗi lần truy vấn.</div>
                    </div>
                    <div class='hero-stat'>
                        <div class='hero-stat-label'>Dữ liệu phiên</div>
                        <div class='hero-stat-value'>{cached_count}</div>
                        <div class='hero-stat-sub'>Số bản ghi hiện có trong bộ nhớ phiên.</div>
                    </div>
                    <div class='hero-stat'>
                        <div class='hero-stat-label'>Phong cách giao diện</div>
                        <div class='hero-stat-value'>Premium</div>
                        <div class='hero-stat-sub'>Giữ nguyên tính năng cũ, nâng cấp trải nghiệm trình bày.</div>
                    </div>
                </div>
            </div>
        </div>
...
with st.sidebar:
    st.markdown(
        ...
        <div class='sidebar-shell'>
            ...
            <div class='sidebar-desc'>
                Giao diện cao cấp cho truy vấn NCBI, phân tích FASTA, alignment và trực quan hóa dữ liệu gen nông nghiệp.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    st.divider()
    st.caption(
        "Luồng sử dụng gợi ý: cấu hình hệ thống → truy vấn gen → phân tích trình tự → alignment → visualize."
    )
...
if page == "Khám phá Gen":
    ...
    render_hero(
        ...
        "Tổ chức khu vực tìm kiếm theo phong cách sản phẩm khoa học cao cấp, giúp app trông gọn, rõ và chuyên nghiệp hơn khi demo hoặc thuyết trình.",
        badges=["NCBI Entrez", "Metadata xuất chuẩn", "Gene discovery", "Premium UI"],
    )
    ...
    with left:
        ...
        st.markdown(
            "<div class='tiny-note'>Nhập từ khóa gene, chọn loài mục tiêu và loại sequence để truy vấn nhanh dữ liệu từ NCBI.</div>",
            unsafe_allow_html=True,
        )

        query = st.text_input("Từ khóa gen", placeholder="Ví dụ: Pi54, rbcL, drought resistance...")
        top_agri_organisms = [
            "",
            "Oryza sativa",
            "Zea mays",
    ...
    with right:
        st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
        render_section("Tổng quan truy vấn")
        if isinstance(records, pd.DataFrame) and not records.empty:
            species_count = records["Organism"].nunique() if "Organism" in records.columns else 0
            dominant_type = safe_mode(records["Gene Type"]) if "Gene Type" in records.columns else "N/A"
            st.metric("Số bản ghi đang có", len(records))
            st.metric("Số loài", species_count)
            st.metric("Loại phổ biến", dominant_type)
        else:
            st.info("Chưa có dữ liệu phiên. Hãy thực hiện truy vấn đầu tiên để hiển thị thống kê nhanh.")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if not st.session_state.get("ncbi_email"):
        ...
        if not query and not organism:
            st.error("Vui lòng nhập ít nhất từ khóa gene hoặc tên loài.")
        else:
    ...
    if isinstance(df_records, pd.DataFrame) and not df_records.empty:
        ...
        with tabs[0]:
            ...
            st.markdown(
                "<div class='premium-callout'>Khối tổng quan này rất hợp để trình bày nhanh quy mô dữ liệu, độ đa dạng loài và loại sequence nổi bật của lần truy vấn.</div>",
                unsafe_allow_html=True,
            )
...
# =========================
    ...
    render_hero(
        "Phòng thí nghiệm trình tự",
        "Biến phần phân tích FASTA thành một không gian có cảm giác như công cụ nghiên cứu thật: sáng sủa, có nhịp trình bày và chuyên nghiệp hơn nhiều.",
        badges=["FASTA", "ORF", "Protein translation", "Nucleotide profile"],
    )

    input_method = st.radio("Chế độ nhập", ["Dán FASTA", "Nhập Accession ID"], horizontal=True)
    seq_data = ""
    header = ""
    ...
    with left:
        ...
        if input_method == "Dán FASTA":
            ...
            acc_id = st.text_input("Accession ID", placeholder="Ví dụ: JN005831")
            ...
            if load_target:
                with st.spinner(f"Đang tải accession {load_target}..."):
    ...
    with right:
        st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
        render_section("Hướng dẫn nhanh")
        st.markdown(
            """
            <div class='premium-callout'>
            Bạn có thể:
            <br>• Dán trực tiếp FASTA để phân tích nhanh
            <br>• Hoặc nhập Accession ID để lấy dữ liệu từ NCBI
            <br>• Hệ thống sẽ tính thống kê cơ bản, phát hiện ORF, dịch protein và cho tải báo cáo
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    if seq_data:
        if not validate_sequence(seq_data):
            st.error("Trình tự không hợp lệ. Chỉ chấp nhận các nucleotide cơ bản A, T, G, C, N, U.")
        else:
            stats = get_basic_stats(seq_data)
            orfs = find_orfs(seq_data)

            st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
            render_section("Thống kê cơ bản")
            ...
            c4.metric("Số ORF (>=100nt)", len(orfs))
            st.markdown("</div>", unsafe_allow_html=True)
            ...
            with plot_col:
                ...
                st.plotly_chart(
                    apply_plot_theme(plot_nucleotide_composition(stats)),
                    use_container_width=True
                )
                st.markdown("</div>", unsafe_allow_html=True)

            with right_col:
                tabs = st.tabs(["Bảng ORF", "Dịch Protein", "Xuất báo cáo"])

                with tabs[0]:
                    st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
                    render_section("Danh sách ORF")
                    if orfs:
                        df_orfs = pd.DataFrame(orfs, columns=["Start (nt)", "End (nt)", "Frame"])
                        st.dataframe(df_orfs, use_container_width=True, height=340)
                    else:
                        st.info("Không tìm thấy ORF phù hợp.")
                    st.markdown("</div>", unsafe_allow_html=True)

                with tabs[1]:
                    st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
                    render_section("Dịch DNA sang Protein")
                    if st.button("Dịch protein", type="primary"):
                        protein_seq = translate_sequence(seq_data)
                        st.text_area("Chuỗi axit amin", value=protein_seq, height=220)
                    else:
                        st.markdown(
                            "<div class='premium-callout'>Nhấn nút để dịch trình tự DNA sang protein và dùng phần này như điểm nhấn khi thuyết trình pipeline phân tích sinh học phân tử.</div>",
                            unsafe_allow_html=True,
                        )
                    st.markdown("</div>", unsafe_allow_html=True)
...
# PAGE 3: ALIGNMENT
# =========================
elif page == "Căn chỉnh trình tự":
    render_hero(
        "Căn chỉnh trình tự",
        "Giữ nguyên thuật toán và chức năng cũ, nhưng trình bày theo giao diện sạch và cao cấp hơn để kết quả alignment nhìn chuyên nghiệp hơn khi demo.",
        badges=["Needleman-Wunsch", "Smith-Waterman", "Dynamic Programming", "Pairwise alignment"],
    )

    st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
    render_section("Nhập hai trình tự", "🧾")

    col_seq1, col_seq2 = st.columns(2)
    with col_seq1:
        seq1 = st.text_area("Trình tự 1 (DNA/RNA)", placeholder="VD: GATTCGA", height=180)
    with col_seq2:
        seq2 = st.text_area("Trình tự 2 (DNA/RNA)", placeholder="VD: GACTCGA", height=180)

    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
    render_section("Thông số thuật toán", "⚙️")
    ...
    run_alignment = st.button("Bắt đầu căn chỉnh", type="primary", use_container_width=True)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if run_alignment:
        ...
        if not seq1 or not seq2:
            ...
            st.warning("Chiều dài tối đa hỗ trợ trên demo là 1000 bp để tránh timeout.")
        elif not validate_sequence(seq1) or not validate_sequence(seq2):
            st.error("Trình tự không hợp lệ.")
        else:
            with st.spinner("Đang tính toán ma trận quy hoạch động..."):
                ...
                st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
                render_section("Kết quả alignment", "🧬")
                st.success(f"Alignment Score: {score}")

                formatted_html = f"""
                <div class='premium-callout'>
                    <pre style='background-color:#08120E; padding:16px; border-radius:14px; color:#F3FFF7; font-family:monospace; line-height:1.7;'>
Seq1: {al1}
      {match_line}
Seq2: {al2}
                    </pre>
...
# =========================
    ...
    render_hero(
        "Bảng điều khiển sinh học",
        "Tập trung vào bố cục có chiều sâu thị giác, phù hợp với trình bày học thuật: nhìn vào là thấy dữ liệu, tầng thông tin và cách kể chuyện khoa học rõ ràng.",
    ...
    st.markdown(
        "<div class='tiny-note'>Bạn có thể dùng dữ liệu từ tab Khám phá Gen hoặc nạp CSV riêng để trình bày nhiều kịch bản phân tích khác nhau.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if isinstance(df, pd.DataFrame) and not df.empty:
        ...
        c2.metric("Số loài sinh học", df["Organism"].nunique() if "Organism" in df.columns else 0)
        top_gene = safe_mode(df["Gene Type"]) if "Gene Type" in df.columns else "N/A"
        ...
        st.markdown(
            "<div class='premium-callout'>Gợi ý diễn giải: dùng biểu đồ phân bố độ dài để phát hiện ngoại lệ, biểu đồ thành phần loài để nhận diện thiên lệch mẫu, timeline để thấy động lực tăng trưởng dữ liệu, và bản đồ nhiệt GC để thảo luận cấu trúc trình tự giữa các nhóm.</div>",
            unsafe_allow_html=True,
        )
    else:
...
# PAGE 5: SETTINGS
# =========================
elif page == "Cài đặt hệ thống":
    render_hero(
        "Cài đặt hệ thống",
        "Giữ phần cấu hình gọn nhưng sang: đủ độ tin cậy để trình bày như một sản phẩm hoàn thiện thay vì một bản demo vội.",
    ...
    with left:
        ...
        st.markdown(
            "<div class='tiny-note'>NCBI yêu cầu email để sử dụng E-utilities. API key là tùy chọn nhưng giúp tăng giới hạn request.</div>",
            unsafe_allow_html=True,
        )
    ...
    with right:
        ...
        st.metric("API Key", "Đã lưu" if st.session_state["ncbi_api_key"] else "Trống")
        st.metric("Số kết quả tối đa", st.session_state["max_results"])
        st.markdown("</div>", unsafe_allow_html=True)

        st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
        render_section("Lưu ý sử dụng", "📎")
        st.markdown(
            """
            <div class='premium-callout'>
            • Hãy cấu hình Email NCBI trước khi tìm kiếm hoặc tải FASTA từ accession.<br>
            • Các chức năng phân tích FASTA, alignment và dashboard vẫn giữ nguyên như bản cũ.<br>
            • Bản nâng cấp này chỉ tập trung làm giao diện đẹp hơn, hiện đại hơn và phù hợp khi demo/thuyết trình.
            </div>
            """,
            unsafe_allow_html=True,
        )
