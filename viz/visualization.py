...
def plot_submission_timeline(df: pd.DataFrame) -> go.Figure:
    ...
    if timeline.empty:
        # Nếu empty thì tạo df rỗng nhưng đủ cột để khỏi lỗi plotly
        timeline = pd.DataFrame(columns=['Year', 'Count'])
    else:
...
def plot_nucleotide_composition(stats_dict: Dict[str, Any]) -> go.Figure:
    """Vẽ Bar chart thành phần nucleotide A/T/G/C/N."""
    bases = ['A', 'T', 'G', 'C', 'N']
    # Tìm key theo bases (định dạng '%A', '%T', hoặc '%N (Unknown)')
    percs = []
    for b in bases:
        if b == 'N':
            percs.append(stats_dict.get('%N (Unknown)', 0))
        else:
            percs.append(stats_dict.get(f'%{b}', 0))
    ...
    fig = px.bar(
        ...
        color_discrete_map={'A': '#1f77b4', 'T': '#ff7f0e', 'G': '#2ca02c', 'C': '#d62728', 'N': '#7f7f7f'}
    )
    fig.update_layout(showlegend=False)
    return fig

def plot_gc_heatmap(df: pd.DataFrame) -> go.Figure:
    """Heatmap minh họa tương quan Loài và Loại Gen."""
